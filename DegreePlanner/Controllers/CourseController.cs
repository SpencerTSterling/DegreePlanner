...
namespace DegreePlanner.Controllers
{
    public class CourseController : Controller
    {
        ...
        #region Create/Add Course
        public IActionResult Create()
        {
            // Get a list of the terms
            IEnumerable<SelectListItem> TermList = _uow.Term.GetAll().Select(u => new SelectListItem
            {
                Text = u.Name,
                Value = u.Id.ToString()
            });
        ...
        public IActionResult Edit(Course obj) 
        {
            ...
            var terms = _uow.Term.GetAll();
            ViewBag.TermList = new SelectList(terms, "Id", "Name"); // Show the list of Terms
            ...
            if (ModelState.IsValid)
            {
                // Retrieve the selected term based on the TermId
                var selectedTerm = _uow.Term.Get(t => t.Id == obj.TermId);

                // Ensure that the selected term exists
                if (selectedTerm == null)
                {
                    ModelState.AddModelError("TermId", "Selected term is invalid.");
                    return View(obj);
                }

                // Set the Term property of the Course object
                obj.Term = selectedTerm;


                _uow.Course.Update(obj);
                _uow.Save();
                // Success notification
